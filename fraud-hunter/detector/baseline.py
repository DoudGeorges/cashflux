...
def build_baselines(df: pd.DataFrame) -> dict:
    ...
    for card_id, group in df.groupby("card_id"):
        ...
        baselines[card_id] = {
            ...
            "iqr": float(amounts.quantile(0.75) - amounts.quantile(0.25)) if len(amounts) else 1,
            "p95": float(amounts.quantile(0.95)) if len(amounts) else 0,
            "typical_categories": set(debits["merchant_category"].value_counts().head(3).index),
...
def build_cross_card_maps(df: pd.DataFrame) -> dict:
    """
