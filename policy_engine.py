...
from paths import POLICY_DOC_PATH, RULES_PATH
from formatting import EMPTY_LABEL, fmt_date, sanitize_display_text

ROOT = Path(__file__).resolve().parent

RESTAURANT_MCC = {5812, 5813, 5814}
...
DEFAULT_RULES = {
    ...
    "team_meal_keywords": ["team", "group", "client", "customer", "dinner with", "lunch with"],
    "customer_keywords": ["customer", "client", "guest"],
    "personal_expense_keywords": ["personal", "family", "spouse", "gift card"],
    "prohibited_expense_keywords": ["traffic ticket", "parking ticket", "personal use", "fine"],
    "irrelevant_business_keywords": [
        "blind box", "blindbox", "pop mart", "popmart", "labubu", "gacha", "collectible",
        "figurine", "funko", "plush", "trading card", "pokemon", "mystery box", "surprise box",
        "toy", "lottery", "steam game", "playstation", "xbox", "anime merch",
    ],
...
DEPT_OVERRIDE_FIELDS = [
    {"key": "manager_approval_threshold", "label": "Approval threshold", "type": "number"},
    ...
    {"key": "conference_pre_approved", "label": "Conferences pre-approved", "type": "boolean"},
]

POLICY_SCHEMA = [
    {"key": "pre_auth_threshold", "label": "Pre-authorization threshold", "type": "number", "section": "thresholds", "unit": "$", "help": "Expenses above this require manager pre-auth."},
    {"key": "manager_approval_threshold", "label": "Manager approval threshold", "type": "number", "section": "thresholds", "unit": "$", "help": "Expenses above this require formal approval."},
    {"key": "solo_meal_limit", "label": "Solo meal limit", "type": "number", "section": "meals", "unit": "$", "help": "Max for a meal without team/client context."},
    {"key": "team_meal_limit", "label": "Team meal total limit", "type": "number", "section": "meals", "unit": "$", "help": "Max total for documented team meals."},
    {"key": "team_meal_per_person_limit", "label": "Team meal per-person limit", "type": "number", "section": "meals", "unit": "$", "help": "Estimated per-person cap on team meals."},
    {"key": "solo_meal_flag_minimum", "label": "Solo meal flag minimum", "type": "number", "section": "meals", "unit": "$", "help": "Only flag solo meals above this amount."},
    {"key": "tip_max_percent", "label": "Max meal tip", "type": "number", "section": "meals", "unit": "%", "help": "Meal tips reimbursed up to this percent."},
    {"key": "service_tip_max_percent", "label": "Max service tip", "type": "number", "section": "meals", "unit": "%", "help": "Porterage/services tip cap."},
    {"key": "receipt_required", "label": "Receipts required", "type": "boolean", "section": "general", "help": "All reimbursements require receipts."},
    {"key": "receipt_submission_days", "label": "Receipt submission window", "type": "number", "section": "general", "unit": "days", "help": "Submit receipts within this many days."},
    {"key": "personal_expenses_prohibited", "label": "Personal expenses prohibited", "type": "boolean", "section": "general", "help": "Block personal spend on corporate cards."},
    {"key": "split_purchase_window_hours", "label": "Split purchase window", "type": "number", "section": "fraud", "unit": "hours", "help": "Look for split charges within this window."},
    {"key": "split_purchase_min_charges", "label": "Split purchase min charges", "type": "number", "section": "fraud", "help": "Minimum charges to trigger split detection."},
    {"key": "mileage_rate_per_km", "label": "Mileage rate", "type": "number", "section": "travel", "unit": "$/km", "help": "CRA mileage reimbursement rate."},
    {"key": "alcohol_keywords", "label": "Alcohol keywords", "type": "keywords", "section": "keywords", "help": "Flag unless customer entertainment documented."},
    {"key": "team_meal_keywords", "label": "Team meal keywords", "type": "keywords", "section": "keywords", "help": "Indicate team/client meal context."},
    {"key": "customer_keywords", "label": "Customer keywords", "type": "keywords", "section": "keywords", "help": "Allow alcohol when these appear on receipt."},
    {"key": "personal_expense_keywords", "label": "Personal expense keywords", "type": "keywords", "section": "keywords", "help": "Flag likely personal purchases."},
    {"key": "irrelevant_business_keywords", "label": "Non-business expense keywords", "type": "keywords", "section": "keywords", "help": "Flag purchases unrelated to company work (collectibles, blind boxes, toys, etc.)."},
    {"key": "prohibited_expense_keywords", "label": "Prohibited expense keywords", "type": "keywords", "section": "keywords", "help": "Never reimbursable (tickets, fines, etc.)."},
    {"key": "restricted_merchants", "label": "Restricted merchants", "type": "keywords", "section": "keywords", "help": "Vendor names that always flag."},
    {"key": "restaurant_mcc_codes", "label": "Restaurant MCC codes", "type": "numbers", "section": "meals", "help": "Merchant category codes treated as restaurants."},
]
...
def policy_summary_text() -> str:
    ...
    lines = [
        ...
        f"- Receipts required within {int(rules['receipt_submission_days'])} days" if rules.get("receipt_required") else "- Receipts optional",
        "- Personal expenses prohibited on corporate cards" if rules.get("personal_expenses_prohibited") else "- Personal expense monitoring relaxed",
...
def _is_restaurant(row, rules: dict | None = None) -> bool:
    ...
    mcc_codes = {int(c) for c in rules.get("restaurant_mcc_codes") or RESTAURANT_MCC}
    mcc = str(row.get("Merchant Category Code") or "").strip()
    if mcc.isdigit() and int(mcc) in mcc_codes:
        return True
    blob = _text_blob(row)
    return any(k in blob for k in ("restaurant", "cafe", "dining", "grill", "bistro", "kitchen"))
...
def _team_meal_context(row, df: pd.DataFrame | None = None) -> tuple[bool, str]:
    ...
    if df is not None and "Transaction Date" in df.columns:
        ...
        if pd.notna(date) and merchant:
            ...
            if employees >= 2:
                return True, f"{employees} employees charged same merchant within 6h (likely team meal)"
...
def check_transaction_policy(row, rules: dict | None = None, df: pd.DataFrame | None = None) -> list[dict]:
                ...
                violations.append(
                    {
                        ...
                        "severity": "Medium",
                        "reason": (
                            f"${amount:,.2f} team meal ({team_note}; ~{party} people, "
                            f"~${est_per:,.0f}/person) exceeds policy limits."
                        ),
                        "employee": employee,
                        "department": dept,
                        "vendor": vendor,
    ...
    if row_rules.get("receipt_required") and amount >= float(row_rules.get("pre_auth_threshold", 50)):
        covered = _receipt_covered_keys()
        tx_date = row.get("Transaction Date")
        date_str = tx_date.date().isoformat() if hasattr(tx_date, "date") and pd.notna(tx_date) else ""
...
def detect_split_purchases(df: pd.DataFrame, rules: dict | None = None) -> list[dict]:
    ...
    for (emp_id, merchant), group in work.groupby(
        ["Employee ID", work["Merchant Info DBA Name"].astype(str).str.strip().str.lower()]
    ):
...
def scan_all_policy_violations(df: pd.DataFrame | None = None) -> list[dict]:
    ...
    from guardian_data import transactions, SCORED_TX_PATH
    ...
    if _policy_cache is not None and _policy_cache_mtime == mtime and df is transactions():
...
def get_repeat_offenders(limit: int = 10) -> list[dict]:
    ...
    for name, stats in counts.items():
        ...
        stats["rank_score"] = stats["violations"] * 3 + stats["severe"] * 5 + stats["split_purchases"] * 8
        ranked.append(stats)
...
def merge_flags_with_policy(guardian_flags: list[dict], policy_violations: list[dict]) -> list[dict]:
