"""Site-wide context for the voice assistant. Covers every page and data source in one snapshot."""

from __future__ import annotations

SITE_PAGES = {
    "overview": "Home: dashboard, quick actions, and spend overview",
    "people": "People: employee roster, profiles, and spending comparisons",
    ...
    "budget": "Budgets: department caps, burn rate, and forecasts",
    "map": "Map: purchase locations worldwide",
    "alerts": "Policy flags: violations, repeat offenders, and flagged purchases",
    "approvals": "Review queue: approve spending requests, trip reports, proposals, and fraud flags",
    "reports": "Trip reports: bundled trip expenses for CFO review",
...
def get_site_snapshot(current_view: str | None = None) -> dict:
    ...
    if _snapshot_cache and now - _snapshot_cache_at < _SNAPSHOT_TTL:
        ...
        snap["current_view_label"] = SITE_PAGES.get(
            snap["current_view"], SITE_PAGES["overview"]
        )
        return snap
    ...
    from services.expenses import load_expenses
    ...
    result = {
        ...
        "current_view_label": SITE_PAGES.get(
            current_view or "overview", SITE_PAGES["overview"]
        ),
        "pages": SITE_PAGES,
        "counts": {
        ...
        "totals": {
            "spend_fmt": fmt_money(total_spend),
            "flagged_fmt": fmt_money(flagged_spend),
            "flagged_count": len(flagged_rows),
        },
        "top_departments": [
            {"name": name, "spend_fmt": fmt_money(amt)} for name, amt in top_depts
        ],
    ...
    _snapshot_cache = {
        ...
        if k not in ("current_view", "current_view_label")
    }
...
def build_voice_context_block(current_view: str | None = None) -> str:
    ...
    if snap.get("budget_caps"):
        lines += ["", "DEPARTMENT BUDGET CAPS (current quarter):"]
    ...
    lines += [
        ...
        " : Update spending policy rules: meal limits, approval thresholds, and keywords (admin)",
        " : List and approve, deny, or escalate Review queue items: expenses, flags, fraud, and projects (admin)",
