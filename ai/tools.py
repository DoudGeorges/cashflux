"""Friday assistant tools: navigate, modify budgets, policy, and review queue items, and manage proposals."""
...
from ai.context import SITE_PAGES, get_site_snapshot
...
def tool_declarations() -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "parameters": {
                ...
                "properties": {
                    "rule_key": {
                        "type": "string",
                        "description": "Rule key e.g. solo_meal_limit",
                    },
                    "value": {
                        "description": "New value. Can be a number, a boolean, or a comma-separated list of keywords for list-type rules.",
                    },
                },
        ...
        },
        {
            ...
            "description": "Lists items waiting in the Review queue: approvals, flags, fraud, and project proposals (admin only).",
            "parameters": {
                "type": "object",
                "properties": {
                    "kind": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "budget_source": {
                        "type": "string",
                        "enum": ["existing", "extra"],
                        "description": "Funding source. Use 'existing' to draw from the department budget, or 'extra' to request additional funds.",
                    },
                    "colleagues": {
                        "type": "array",
                        "items": {"type": "string"},
...
def _find_review_item(ctx: FridayContext, item_id: str) -> dict | None:
    ...
    queue = build_review_queue(
        approval_exclude=ctx.decided_keys_fn("approval")
        if ctx.decided_keys_fn
        else set(),
        fraud_status_overrides=ctx.fraud_overrides_fn()
        if ctx.fraud_overrides_fn
        else {},
...
def _perform_review_action(
    ctx: FridayContext, item_id: str, action: str, note: str | None = None
) -> dict:
    from datetime import datetime

    from extensions import db
    from models import ProjectProposal, WorkflowDecision
    ...
    if item_id.startswith("approval:"):
        ...
        db.session.add(
            WorkflowDecision(
                item_key=item_key, item_type="approval", status=status, note=note
            )
        )
        db.session.commit()
        _invalidate(ctx)
    ...
    if item_id.startswith("flag:"):
        ...
        db.session.add(
            WorkflowDecision(
                item_key=item_key, item_type="flag", status=status, note=note
            )
        )
        db.session.commit()
    ...
    if item_id.startswith("fraud:"):
        ...
        _invalidate(ctx)
        ctx.actions.append(
            {"type": "review_updated", "view": "approvals", "reviewFilter": "fraud"}
        )
    ...
    if item_id.startswith("report:"):
        ...
        _invalidate(ctx)
        ctx.actions.append(
            {"type": "review_updated", "view": "approvals", "reviewFilter": "report"}
        )
        return _ok(status=status, item_id=item_id)
...
def dispatch_tool(ctx: FridayContext, name: str, args: dict | None) -> dict:
    ...
    if name == "navigate":
        ...
        if ctx.is_admin and view in ("receipts", "proposals"):
            return _err(
                "Receipts and My projects are for employees only. Admins should use Review and All purchases."
            )
        extra = {}
        if args.get("tab"):
            extra["tab"] = args["tab"]
    ...
    if name == "list_spending_rules":
        ...
        if denied:
            return denied
        from services.policy import load_policy_rules, policy_summary_text
    ...
    if name == "update_spending_rule":
        ...
        if denied:
            return denied
        from services.policy import POLICY_SCHEMA, load_policy_rules, save_policy_rules
        ...
        if rule_key not in valid_keys:
            return _err(
                f"Unknown rule. Valid keys include: {', '.join(sorted(valid_keys)[:12])}…"
            )
        ...
        try:
            ...
            if schema["type"] == "number":
                ...
                new_val = (
                    [int(x) for x in items] if schema["type"] == "numbers" else items
                )
            else:
                new_val = raw
        except (TypeError, ValueError):
        ...
        _invalidate(ctx)
        ctx.actions.append(
            {"type": "policy_updated", "view": "settings", "tab": "rules"}
        )
    ...
    if name == "list_pending_reviews":
        ...
        queue = build_review_queue(
            ...
            proposal_items=ctx.pending_proposals_fn()
            if ctx.pending_proposals_fn
            else [],
            flag_exclude=ctx.decided_keys_fn("flag") if ctx.decided_keys_fn else set(),
            report_status_overrides=_report_status_overrides(),
        )
        ...
        return _ok(items=out_items, total=len(queue))
    ...
    if name == "review_item":
        ...
        if not _find_review_item(ctx, item_id):
            return _err(
                "Item not found or already decided. Call list_pending_reviews first to get current item IDs."
            )
    ...
    if name == "list_trip_reports":
        ...
        return _ok(reports=out_reports)
    ...
    if name == "submit_project_proposal":
        if ctx.is_admin:
            return _err("Admins cannot submit proposals. Employees use My projects for this.")
