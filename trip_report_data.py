...
from formatting import fmt_date, fmt_money
from guardian_data import credit_score_for_name, transactions
from policy_engine import scan_all_policy_violations
from proposal_data import employee_department
from workflow_data import build_trip_report_recommendation
...
def _guardian_df_for_keys(keys: list[str], employee_name: str) -> pd.DataFrame:
    ...
    if df.empty:
        return pd.DataFrame()
    mask = df.apply(
        lambda row: guardian_row_key(row) in key_set
    ...
    if subset.empty:
        return subset
    subset["Transaction Date"] = pd.to_datetime(subset["Transaction Date"], errors="coerce")
...
def serialize_submission(
    ...
    return {
        ...
        "total": round(total, 2),
        "total_formatted": fmt_money(total) if report else format_money(0),
        "status": status,
        ...
        "spending_purpose": spending_purpose,
        "project_id": project_id,
        "project_title": project_title or (report.get("project_title") if report else "") or "",
    }
