...
from guardian_data import department_summary, transactions

# Quarterly cap = prior quarter spend × this factor when user has not set a budget.
...
def _saved_quarters() -> list[tuple[str, dict]]:
    ...
    return sorted(quarters.items(), key=lambda item: _quarter_sort_key(item[0]), reverse=True)
...
def _saved_budget_quarter(dept: str, preferred_quarter: str | None = None) -> str | None:
    dept_key = str(dept).strip()
...
def get_user_budget(dept: str, quarter: str) -> float | None:
    ...
    for q_label in ([quarter] if quarter else []) + [q for q, _ in _saved_quarters() if q != quarter]:
...
def apply_extra_budget_approval(
    ...
    return {
        ...
        "new_cap": new_cap,
        "extra_amount": extra,
        "previous_cap_fmt": _fmt_money(previous_cap),
        "new_cap_fmt": _fmt_money(new_cap),
    }
...
def resolve_department_name(name: str) -> str | None:
    ...
    if not query:
        return None
    from guardian_data import department_summary
...
def _quarter_total_spend(df: pd.DataFrame, start: datetime, end: datetime) -> float:
    mask = (df["Transaction Date"] >= start) & (df["Transaction Date"] < end)
    return float(df.loc[mask, "Amount Clean"].sum())


def _reference_date(df: pd.DataFrame) -> datetime:
...
def _dept_spend_in_range(df: pd.DataFrame, dept: str, start: datetime, end: datetime) -> float:
    dept_key = str(dept).strip()
    mask = (
...
def quarterly_budget_cap(dept: str, df: pd.DataFrame, ref: datetime, quarter: str | None = None) -> float:
...
def _quarterly_budget_cap(dept: str, df: pd.DataFrame, ref: datetime) -> float:
    return quarterly_budget_cap(dept, df, ref)
...
def get_budget_settings() -> dict:
    ...
    saved = load_budget_config().get("quarters", {}).get(quarter, {})
    ...
    for row in department_summary().sort_values("department").itertuples():
        ...
        departments.append({
            ...
            "budget_fmt": _fmt_money(budget),
            "auto_budget_fmt": _fmt_money(auto),
            "spent_fmt": _fmt_money(spent),
        })
...
def format_budget_caps_for_context(departments: list[str] | None = None) -> list[str]:
    ...
    for row in data["departments"]:
        ...
        lines.append(
            ...
            f"remaining {_fmt_money(remaining)} — {source}"
        )
    return lines
...
def lookup_department_budget(department: str) -> dict | None:
    ...
    return {
        ...
        "remaining_fmt": _fmt_money(remaining),
...
def _week_start_date(q_start: datetime, week_num: int) -> datetime:
    return q_start + timedelta(days=(week_num - 1) * 7)


def _week_date_label(q_start: datetime, week_num: int | None, *, full: bool = False) -> str | None:
    if not week_num or week_num < 1 or week_num > 52:
        return None
    label = _week_start_date(q_start, week_num)
    return label.strftime("%b %d, %Y") if full else label.strftime("%b %d")


def _week_labels(q_start: datetime, weeks_total: int) -> list[str]:
    return [_week_date_label(q_start, w) or "" for w in range(1, weeks_total + 1)]


def _find_exceed_week(
    cap: float,
    ...
    """First week (1-based) when actual or projected cumulative spend reaches the cap."""
    if cap <= 0:
        return None

    for w in range(1, min(current_week, weeks_total) + 1):
        if cumulative[w - 1] >= cap:
            return w
    ...
    if spent >= cap:
        return min(current_week, weeks_total)

    weeks_ahead = (cap - spent) / weekly_burn
    cross = current_week + int(math.ceil(weeks_ahead - 1e-9))
    if cross <= weeks_total:
        return cross
    return None


def _exceed_week_from_chart_series(
    cap: float,
    actual: list[float | None],
    projected: list[float | None],
) -> int | None:
    """Match the red exceed marker to the chart's actual/projected lines."""
...
def forecast_department(
    ...
    df["Transaction Date"] = pd.to_datetime(df["Transaction Date"], errors="coerce")
    df["Amount Clean"] = pd.to_numeric(df["Amount Clean"], errors="coerce").fillna(0).abs()

    ref = ref or _reference_date(df)
    q_start, q_end, quarter, _ = _quarter_bounds(ref)
    spent = _dept_spend_in_range(df, dept, q_start, q_end)
    budget = quarterly_budget_cap(dept, df, ref, quarter)
    ...
    actual_daily, projected_daily, projected_eoq = _project_cumulative_from_history(daily_cumulative, today_idx)
    ...
    exceed_date = (
        (q_start + timedelta(days=exceed_day - 1)).strftime("%b %d, %Y") if exceed_day else None
    )
    ...
    if budget <= 0:
        ...
        message = (
            ...
            f"({_fmt_money(projected_eoq)} projected vs {_fmt_money(budget)} cap)."
        )
    elif spent >= budget:
        outcome = "exceeded"
        message = (
            f"{dept} has already exceeded its {quarter} budget "
            f"({_fmt_money(spent)} of {_fmt_money(budget)}), crossing the cap around {exceed_date}."
        )
    elif exceed_day and exceed_day > today_idx + 1:
        outcome = "projected_exceed"
        message = (
            f"Based on recent spending, {dept} will exceed its {quarter} budget around {exceed_date}."
        )
    else:
        outcome = "exceeded"
        message = (
            f"{dept} is over {quarter} budget pace — would have crossed the cap around {exceed_date}."
        )
    ...
    return {
        ...
        "weekly_burn_fmt": _fmt_money(weekly_burn),
        ...
        "projected_eoq_fmt": _fmt_money(projected_eoq),
        "outcome": outcome,
        "message": message,
        "budget_fmt": _fmt_money(budget),
        "spent_fmt": _fmt_money(spent),
...
def get_department_forecasts(limit: int = 8) -> list[dict]:
    ...
    forecasts = [forecast_department(str(row.department), df, ref) for row in dept_df.itertuples()]
...
def get_forecast_for_query(query: str) -> dict | None:
    q = query.lower()
    if not any(k in q for k in ("forecast", "burn rate", "burn", "exceed", "run out", "over budget", "project")):
        return None

    dept_df = department_summary()
    dept = None
    for name in sorted(dept_df["department"].astype(str).tolist(), key=len, reverse=True):
        if name.lower() in q:
            dept = name
            break
    ...
    at_risk = [f for f in get_department_forecasts(limit=20) if f["outcome"] in ("projected_exceed", "exceeded")]
    if at_risk:
        return at_risk[0]
...
def _daily_cumulative_spend(
    ...
    for d in range(total_days):
        ...
        if day_start >= q_end:
            ...
            continue
        dmask = (qdf["Transaction Date"] >= day_start) & (qdf["Transaction Date"] < day_end)
        running += float(qdf.loc[dmask, "Amount Clean"].sum())
        cumulative.append(round(running, 2))
    return cumulative


def _daily_labels(q_start: datetime, total_days: int, today_idx: int, ref: datetime) -> list[str]:
...
def _project_cumulative_from_history(
    ...
    recent_daily = (
        sum(recent_daily_window) / len(recent_daily_window) if recent_daily_window else 0.0
    )
    ...
    for i in range(today_idx + 1, total_days):
        if (i - today_idx) % 7 == 0:
            week_counter += 1
            weekly_est = _projected_weekly_increment(weekly_inc) if weekly_inc else daily_step * 7
...
def _weekly_cumulative_spend(
    ...
    """Actual cumulative spend by week within the quarter."""
    mask = (
        (df["Department"] == dept)
        & (df["Transaction Date"] >= q_start)
        & (df["Transaction Date"] < q_end)
    ...
    for w in range(1, weeks_total + 1):
        week_start = q_start + timedelta(days=(w - 1) * 7)
        week_end = min(q_start + timedelta(days=w * 7), q_end)
        if week_start >= q_end:
            cumulative.append(round(running, 2))
            continue
        wmask = (dept_df["Transaction Date"] >= week_start) & (dept_df["Transaction Date"] < week_end)
        running += float(dept_df.loc[wmask, "Amount Clean"].sum())
        cumulative.append(round(running, 2))
    return cumulative
...
def _weekly_cumulative_spend_all(
    df: pd.DataFrame,
    q_start: datetime,
    q_end: datetime,
    weeks_total: int,
) -> list[float]:
    """Actual cumulative spend by week across all departments."""
    mask = (df["Transaction Date"] >= q_start) & (df["Transaction Date"] < q_end)
    qdf = df.loc[mask]
    cumulative: list[float] = []
    running = 0.0
    for w in range(1, weeks_total + 1):
        ...
        if week_start >= q_end:
            cumulative.append(round(running, 2))
            continue
        wmask = (qdf["Transaction Date"] >= week_start) & (qdf["Transaction Date"] < week_end)
        running += float(qdf.loc[wmask, "Amount Clean"].sum())
        cumulative.append(round(running, 2))
    return cumulative
...
def forecast_company_wide(
    ...
    total_days = _days_in_quarter(q_start, q_end)
    today_idx = _day_index(q_start, ref, total_days)
    daily_cumulative = _daily_cumulative_spend("All departments", df, q_start, q_end, total_days)
    actual_daily, projected_daily, projected_eoq = _project_cumulative_from_history(daily_cumulative, today_idx)
    ...
    if total_budget <= 0:
        ...
        finish_pct = round((projected_eoq / total_budget) * 100, 0) if total_budget else 0
        message = (
            f"Based on recent spending, combined spend will finish {quarter} at {finish_pct}% of budget "
            f"({_fmt_money(projected_eoq)} projected vs {_fmt_money(total_budget)} cap)."
        )
    elif total_spent >= total_budget:
        outcome = "exceeded"
        message = (
            f"Company spend has already exceeded the combined {quarter} budget "
            f"({_fmt_money(total_spent)} of {_fmt_money(total_budget)}), crossing the cap around {exceed_date}."
        )
    elif exceed_day and exceed_day > today_idx + 1:
        outcome = "projected_exceed"
        message = (
            f"Based on recent spending, combined spend will exceed the {quarter} budget around {exceed_date}."
        )
    else:
        outcome = "exceeded"
        message = (
            f"Combined spend is over {quarter} budget pace — would have crossed the cap around {exceed_date}."
        )
    ...
    return {
        ...
        "outcome": outcome,
        "message": message,
        "budget_fmt": _fmt_money(total_budget),
        "spent_fmt": _fmt_money(total_spent),
        "remaining_fmt": _fmt_money(remaining),
    }
...
def _forecast_chart(
    ...
    actual, projected, projected_eoq = _project_cumulative_from_history(daily_cumulative, today_idx)
    ...
    return {
        ...
        "today_index": today_idx,
        "days_in_quarter": total_days,
        "current_week_label": labels[today_idx] if 0 <= today_idx < total_days else None,
        ...
        "exceed_week_label": labels[exceed_day - 1] if exceed_day and 1 <= exceed_day <= total_days else None,
        "exceed_date": (q_start + timedelta(days=exceed_day - 1)).strftime("%b %d, %Y") if exceed_day else None,
...
def get_budget_overview() -> dict:
    ...
    for row in dept_df.itertuples():
        ...
        if fc["status"] in ("warn", "critical") or fc["outcome"] in ("projected_exceed", "exceeded"):
            at_risk += 1
    ...
    month_values = []
    if not qdf.empty:
        monthly = qdf.groupby(qdf["Transaction Date"].dt.to_period("M"))["Amount Clean"].sum()
    ...
    return {
        ...
        "summary": {
            ...
            "total_budget_fmt": _fmt_money(total_budget),
            "total_spent_fmt": _fmt_money(total_spent),
            "total_remaining_fmt": _fmt_money(total_remaining),
