...
_company_id: contextvars.ContextVar[int | None] = contextvars.ContextVar("company_id", default=None)
_company_slug: contextvars.ContextVar[str | None] = contextvars.ContextVar("company_slug", default=None)
...
def get_company_paths(company_id: int | None = None, slug: str | None = None) -> CompanyPaths:
...
def init_empty_company_data(company_id: int, slug: str) -> CompanyPaths:
    ...
    _write_header_csv(paths.original_tx, headers.get("original_tx") or [
        "Employee ID", "Employee Name", "Department", "Transaction Amount", "Debit or Credit",
    ])
    _write_header_csv(paths.scored_tx, headers.get("scored_tx") or headers.get("original_tx") or [])
    _write_header_csv(paths.employee_scores, headers.get("employee_scores") or [
        "employee_id", "employee_name", "department", "starting_score", "final_score",
        "total_score_change", "total_spent", "reasonable_transactions", "flagged_transactions",
        "geofence_flags", "one_sentence_summary",
    ])
    _write_header_csv(paths.flagged_tx, headers.get("flagged_tx") or headers.get("scored_tx") or [])
    _write_header_csv(paths.department_summary, headers.get("department_summary") or [
        "department", "total_spent", "transaction_count", "reasonable_transactions",
        "flagged_transactions", "geofence_flags", "average_score_after_transactions",
    ])
