...
def roster_employee_names(company_slug: str | None = None) -> list[str]:
    from company_data import ensure_company_data, set_company_context
    from guardian_data import employee_summary
...
def register_auth_routes(app):
    ...
    def signup():
        ...
        error = None
        mode = request.form.get("mode") if request.method == "POST" else request.args.get("mode", "create")
        if mode not in ("create", "join"):
            mode = "create"
        join_slug = (request.args.get("company_slug") or request.form.get("company_slug") or "").strip()
        roster = roster_employee_names(join_slug) if mode == "join" and join_slug else []
        ...
        if request.method == "POST":
            ...
            if not email or not password or len(password) < 6:
                ...
                if not company_name or not display_name:
                    ...
                    _db.session.commit()
                    from company_data import init_empty_company_data
                ...
                if not company:
                    ...
                    if join_roster and (not employee_name or employee_name not in join_roster):
