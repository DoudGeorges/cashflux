...
from services.company import get_company_key, get_company_paths
...
from core.paths import LOCATION_COORDS_PATH, POSTAL_COORDS_PATH, STREET_COORDS_PATH
...
def _apply_unified_flags(rows: list[dict]) -> None:
    ...
    from services.expenses.guardian import flag_item_key, get_flags_list
...
def _normalize_full_address_for_geocode(full_address):
    ...
    for part in parts:
        ...
        if upper == "CAN":
            ...
            part.isupper()
            and sum(ch.isalpha() for ch in part) >= 3
...
def _map_location_label(city, state, country, postal):
    ...
    return (
        ", ".join(part for part in ((city or "").strip(), state, country) if part)
        or "Unknown"
    )
...
def _normalize(raw, emp_meta=None):
    ...
    return {
        ...
        "employee": employee,
        "vendor": (
            raw.get("Merchant Info DBA Name")
            or raw.get("Transaction Description")
            or ""
        ).strip(),
...
def build_gemini_context(
    user_message: str, conversation_history=None, employee_name=None
) -> str:
    ...
    spend_heading = (
        "MY SPEND BY CATEGORY:" if employee_name else "SPEND BY EMPLOYEE (all time):"
    )
    if employee_name:
        lines.append(spend_heading)
        for cat, amt in _top_items(_sum_amounts(rows, "category"), 10):
            lines.append(f"  {cat}: {fmt_money(amt)}")
    else:
        lines.append(spend_heading)
        for emp, amt in _top_items(_sum_amounts(rows, "employee"), 10):
            flagged = sum(
                1 for r in rows if r["employee"] == emp and r.get("flagged") == "yes"
            )
            lines.append(f"  {emp}: {fmt_money(amt)} ({flagged} flagged txns)")

    lines += ["", "SPEND BY CITY (all time):"]
    for city, amt in _top_items(_sum_by_city(rows), 12):
        lines.append(f"  {city}: {fmt_money(amt)}")

    lines += ["", f"TOP VENDORS ({latest_month}):"]
    for vendor, amt in _top_items(_sum_amounts(month_rows, "vendor"), 10):
        lines.append(f"  {vendor}: {fmt_money(amt)}")
    ...
    for country, amt in _top_items(_sum_amounts(rows, "country"), 8):
        lines.append(f"  {country}: {fmt_money(amt)}")

    lines += ["", "FLAGGED / HIGH-VALUE TRANSACTIONS (â‰¥ $500):"]
    ...
    for r in sorted(flagged_rows, key=lambda x: x["amount"], reverse=True)[:20]:
        lines.append(
            ...
            f"{_city_label(r)} | {fmt_money(r['amount'])} | {r['location']}"
        )
    ...
    if emp:
        emp_rows = _filter_rows(rows, employee=emp)
        ...
        for r in sorted(emp_rows, key=lambda x: x["date"], reverse=True)[:25]:
            lines.append(
                ...
                f"{fmt_money(r['amount'])} | {r['location']}"
            )
    ...
    for city_hint in ("las vegas", "montreal", "toronto", "chicago", "calgary"):
        if city_hint in q:
            ...
            for r in sorted(city_rows, key=lambda x: x["date"], reverse=True)[:20]:
                lines.append(
                    f"  {r['date'].date()} | {r['employee']} | {r['vendor']} | {fmt_money(r['amount'])}"
                )
            break
    ...
    if dept:
        ...
        for cat, amt in _top_items(_sum_amounts(dept_rows, "category"), 10):
            lines.append(f"  {cat}: {fmt_money(amt)}")
        if latest_q:
            q_dept = [r for r in dept_rows if r["quarter"] == latest_q]
            total_q = sum(r["amount"] for r in q_dept)
            lines += ["", f"{dept} {latest_q} total: {fmt_money(total_q)}"]
    ...
    try:
        ...
        if not employee_name:
            cap_lines = format_budget_caps_for_context([dept] if dept else None)
            lines += ["", "DEPARTMENT BUDGET CAPS (Settings for current quarter):"]
    ...
    try:
        from services.policy import load_policy_document, policy_summary_text
...
def get_charts_for_query(
    user_message: str, conversation_history=None, employee_name=None
):
    from services.expenses.spending import analyze_spending_query
...
def get_dashboard(employee_name=None):
    rows = scoped_expenses(employee_name)
    by_emp = _sum_amounts(rows, "employee")
    ...
    if employee_name:
        by_dept = _sum_amounts(rows, "department")
        by_cat = _sum_amounts(rows, "category")
        ...
        departments = [
            {
                "department": dept,
                "total_spent_fmt": fmt_money(total),
                "transaction_count": sum(
                    1 for r in rows if r.get("department") == dept
                ),
                "flagged_transactions": sum(
                    1
                    for r in rows
                    if r.get("department") == dept and r.get("flagged") == "yes"
                ),
    ...
    return {
        ...
        "credit_scores": [
            {
                "name": emp,
                "dept": emp_meta.get(emp, {}).get(
                    "department",
                    next(
                        (r["home_department"] for r in rows if r["employee"] == emp),
                        "-",
                    ),
                ),
...
def _employee_summary(rows):
    ...
    return {
        ...
        "top_vendors": [
            {"vendor": v, "amount": fmt_money(a), "raw": a} for v, a in top_vendors
        ],
        ...
        result.append(
            {
                ...
                "department": emp_meta.get(
                    "department", emp_rows[0].get("home_department", "-")
                ),
                "total_spend": total,
                "total_spend_fmt": fmt_money(total),
                ...
                "flagged_count": int(
                    emp_meta.get(
                        "flagged_transactions",
                        sum(1 for r in emp_rows if r.get("flagged") == "yes"),
                    )
                ),
...
def get_employee_detail(name, employee_name=None):
    if employee_name and name != employee_name:
        return None
    rows = scoped_expenses(employee_name)
    emp_rows = _filter_rows(rows, employee=name)
    if not emp_rows:
        return None
...
def compare_employees(names, employee_name=None):
    ...
    for name in names:
        ...
        summary = _employee_summary(emp_rows)
        employees.append(
            {"name": name, "employee_id": emp_rows[0]["employee_id"], **summary}
        )
...
def _geocode_nominatim_address(address):
    ...
    try:
        ...
        req = urllib.request.Request(
            url, headers={"User-Agent": "CashFlux/1.0"}
        )
    ...
    *,
    ...
    if _valid_street(street_address):
        ...
        coords = _geocode_street_cached(
            query, street_address, postal, state, country, api_key
        )
        if coords:
            return coords
    ...
    if _valid_city(city):
        coords = _geocode_location(
            city, state, country, api_key, location_label=location_label
        )
        if coords:
            return coords
...
def get_map_locations(api_key=None, limit=40, employee_name=None):
        ...
        locations.append(
            {
                ...
                "spend_fmt": fmt_money(bucket["spend"]),
    ...
    return {
        ...
        "mapped_spend_fmt": fmt_money(total_spend),
    }


_postal_geocode_cache: dict[str, Any] = {}
...
def _save_street_coords():
    ...
    STREET_COORDS_PATH.write_text(
        json.dumps(_load_street_coords(), indent=2), encoding="utf-8"
    )
    _street_coords_dirty = False


def _geocode_street_cached(
    query, street, postal, state, country, api_key, full_address=None
):
...
def _resolve_merchant_coords(
    site: dict, api_key=None, street_cache=None
) -> tuple[dict | None, str]:
    ...
    if geocode_query and (
        _valid_street(site.get("street_address")) or site.get("full_address")
    ):
        coords = _geocode_street_cached(
            geocode_query,
    ...
    if api_key:
        ...
        if coords:
            return coords, "street" if _valid_street(
                site.get("street_address")
            ) else "geocode"
    ...
    if coords:
        via = (
            "street"
            if sk and sk in street_cache and _street_cache_verified(street_cache[sk])
            else "postal"
        )
        return coords, via
    return None, "missing"
...
def _geocode_streets_for_purchases(purchases, api_key, geocode_budget=60):
    ...
    for purchase in purchases:
        ...
        if cached and _street_cache_verified(cached):
            continue
        if not _valid_street(purchase.get("street_address")) and not purchase.get(
            "full_address"
        ):
            continue
    ...
    for purchase in purchases:
        coords, via = _resolve_merchant_coords(
            purchase, api_key=None, street_cache=street_cache
        )
        if not coords:
            continue
...
def _build_purchase_map_index(api_key=None, geocode_budget=0):
    ...
    for key, group in groups.items():
        ...
        coords, via = _resolve_merchant_coords(
            group, api_key if use_api else None, street_cache
        )
        ...
        purchases.append(
            {
                ...
                else f"{len(employee_list)} employees",
                ...
                "spend_fmt": fmt_money(group["spend"]),
...
def get_map_merchants(
    ...
    merchants = _build_purchase_map_index(
        api_key=api_key, geocode_budget=min(geocode_budget, 40)
    )
    if employee_name:
        merchants = [
            m for m in merchants if employee_name in (m.get("employees") or [])
        ]
