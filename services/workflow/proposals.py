...
from services.expenses.guardian import credit_score_for_name, employees_by_name
...
def build_budget_snapshot(department: str) -> dict:
    from services.workflow.budgets import forecast_department
...
def normalize_colleagues(
    raw, roster: set[str] | None = None, exclude: str | None = None
) -> list[str]:
...
def proposal_review_context(proposal: dict, snapshot: dict) -> list[str]:
    ...
    lines = [
        ...
        f"Requested budget: {fmt_money(amt)} for {quarter}",
        (
    ...
    if source == "existing" and amt > remaining:
        lines.append(f"Exceeds remaining budget by {fmt_money(amt - remaining)}")
    if source == "extra":
...
def serialize_proposal(model, spent: float = 0.0) -> dict:
    ...
    data = {
        ...
        "requested_amount_fmt": fmt_money(model.requested_amount),
...
def approved_project_option(model, spent: float = 0.0) -> dict:
    ...
    return {
        ...
        "requested_amount_fmt": fmt_money(budget),
        "spent": round(spent, 2),
        "spent_fmt": fmt_money(spent),
        "remaining": round(remaining, 2),
        "remaining_fmt": fmt_money(remaining),
...
def proposal_to_review_item(proposal_dict: dict) -> dict:
    ...
    return {
        ...
        "budget_source_label": proposal_dict.get("budget_source_label")
        or budget_source_label(source),
