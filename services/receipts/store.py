"""Receipt persistence and CSV pipeline (confirm â†’ JSON + transactions)."""
...
from services.merchants import MERCHANTS_BY_CATEGORY, MONTREAL_MERCHANTS
...
def apply_dining_context(
    payload: dict, *, party_size, dining_with: list | None, employee_name: str | None
) -> dict:
    """Attach meal party details when saving a restaurant/cafe receipt."""
    from services.workflow.proposals import normalize_colleagues
    ...
    guests = normalize_colleagues(
        dining_with or [], roster=roster, exclude=employee_name
    )
    ...
    if size > 0 and amount > 0:
        ...
        payload["dining_per_person_fmt"] = fmt_money(payload["dining_per_person"])
    return payload
...
def list_receipts(
    *, user_id: int | None = None, employee_name: str | None = None
) -> list[dict]:
...
def _lookup_merchant_address(
    merchant_name: str | None, city: str | None, category: str
) -> tuple[str, str, str, str, str]:
    target = str(merchant_name or "").upper()
    city_str = str(city or "MONTREAL").upper()
    ...
    for pool in (MERCHANTS_BY_CATEGORY.get(category, []), MONTREAL_MERCHANTS):
        for m in pool:
            ...
            if name in target or target in name:
                if str(m["city"]).upper() == city_str or city_str == "MONTREAL":
                    return (
                        str(m["street"]),
                        str(m["city"]),
                        str(m["state"]),
...
def _find_matched_row_index(df: pd.DataFrame, transaction_id: str) -> int | None:
    ...
    for idx, row in df.iterrows():
        ...
        if abs(row_amt - target_amount) > 0.51:
            continue
        row_vendor = (
            str(row.get("Merchant Info DBA Name") or "").upper().replace(" ", "")
        )
        if (
            vendor_key
            and vendor_key not in row_vendor
            and row_vendor[:20] not in vendor_key
        ):
            continue
        return idx
    return None
...
def _build_receipt_row(
    *,
    ...
    return {
        ...
        "Approved Full Address": full_address
        if street
        else make_full_address("", city, state, country, postal),
    }
...
def append_receipt_to_csv(
    *,
    ...
    employee_id, employee_name, department = _resolve_employee(
    ...
    if matched_transaction_id:
        ...
        if match_idx is not None:
            ...
            save_transaction_outputs(
                scored, scores_df, flagged_df, department_df, paths=paths
            )
            return {
                "action": "linked",
...
def refresh_transaction_caches() -> None:
    from services.expenses import reload_expense_cache
    from services.expenses.guardian import clear_cache
...
def confirm_receipt(
    *,
    ...
    try:
        ...
        logging.exception("Failed to append receipt to CSV")
        record["csv_error"] = str(exc)
    else:
        record["csv_action"] = csv_result.get("action") if csv_result else None
        record["csv_transaction_id"] = (
            csv_result.get("transaction_id") if csv_result else None
        )
