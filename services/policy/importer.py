"""Import expense policy from a PDF (or text) via Gemini to digitize rules automatically."""
...
def _schema_prompt_block() -> str:
    lines = [
        "Rule fields to extract (use numbers in CAD unless document says otherwise):"
    ]
    for field in POLICY_SCHEMA:
        ...
        if field["type"] == "boolean":
            ...
            lines.append(
                f"- {field['key']} (number{', ' + unit if unit else ''}): {hint}"
            )
...
def _build_import_prompt(departments: list[str]) -> str:
    dept_list = (
        ", ".join(departments)
        if departments
        else "(none listed: only add overrides explicitly named in the PDF)"
    )
    defaults = {
        k: v
        for k, v in DEFAULT_RULES.items()
        if k not in ("department_overrides", "role_overrides")
    }
...
Instructions:
- Map policy language to the closest matching rule keys (e.g. "$50 pre-authorization" â†’ pre_auth_threshold: 50).
...
def extract_policy_from_document(file_bytes: bytes, mime_type: str) -> dict[str, Any]:
    ...
    try:
        ...
        payload = safe_json(response.text or "")
    except Exception as exc:
        return {"error": f"Policy import failed: {exc}"}

    if not payload.get("rules"):
        return {
            "error": "Could not extract policy rules from the document. Try a clearer PDF or edit rules manually."
        }
...
def summarize_rule_changes(before: dict, after: dict) -> list[str]:
    ...
    for key in sorted(set(before) | set(after)):
        ...
        if old != new:
            ...
            changes.append(f"{label}: {_fmt_value(old)} â†’ {_fmt_value(new)}")
...
def apply_imported_policy(extracted: dict) -> dict[str, Any]:
    ...
    doc = (
        extracted.get("document_markdown") or extracted.get("document") or ""
    ).strip()
    if doc:
