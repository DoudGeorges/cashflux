...
from formatting import sanitize_display_text
...
ELECTRONICS_HINTS = ("electronic", "software", "computer", "apple", "newegg", "best buy")


def _infer_channel(row) -> str:
    vendor = str(row.get("Merchant Info DBA Name") or "").lower()
    desc = str(row.get("Transaction Description") or "").lower()
    if any(k in vendor or k in desc for k in ONLINE_HINTS):
        return "online"
    return "in_person"


def _normalize_category(row) -> str:
    cat = str(row.get("Transaction Category") or "").lower()
    desc = str(row.get("Transaction Description") or "").lower()
    blob = f"{cat} {desc}"
    if "gift" in blob:
        return "gift_card"
    if any(k in blob for k in ELECTRONICS_HINTS):
        return "electronics"
    return cat or "other"
...
def load_mpc_as_fraud_df() -> pd.DataFrame:
    ...
    for idx, row in raw.iterrows():
        ...
        employee_id = str(row.get("Employee ID") or row.get("Employee Name") or "unknown")
...
def review_transaction(transaction_id: str, action: str) -> dict:
    ...
    if action == "dismissed":
        ...
        df.loc[similar.index, "fraud_score"] = df.loc[similar.index, "fraud_score"] * 0.85
...
def export_reviewed_csv() -> tuple[io.BytesIO, str]:
    ...
    export["fraud_review_status"] = export.get("review_status", "pending")
    buf = io.BytesIO()
