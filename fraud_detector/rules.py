...
SUSPICIOUS_MERCHANT_KEYWORDS = ("quickpay", "cryptopay", "wire transfer", "verify account")
...
def rule_velocity(row, df: pd.DataFrame, *, velocity_count: int | None = None) -> tuple[float, str]:
...
def rule_merchant_burst(row, df: pd.DataFrame, *, unique_cards: int | None = None) -> tuple[float, str]:
    ...
    suspicious = any(k in merchant_lower for k in SUSPICIOUS_MERCHANT_KEYWORDS)
    ...
    if unique_cards >= 7 and not suspicious:
        return 0.45, f"Merchant '{merchant}' hit {unique_cards} different cards within 24 hours"
    return 0.0, ""
...
def rule_duplicate_charge(row, df: pd.DataFrame) -> tuple[float, str]:
    ...
    if len(dupes) >= 2:
        return 0.75, f"Duplicate ${amount:,.2f} charge at {merchant} within 48h on same card"
    return 0.0, ""
...
def rule_round_number(row, baseline: dict) -> tuple[float, str]:
    ...
    if amount < 100:
        return 0.0, ""
    if amount % 100 == 0:
        return 0.35, f"Round-number amount ${amount:,.0f} — common fraud / split pattern"
...
def rule_suspicious_merchant(row, baseline: dict, *, burst_cards: int = 0) -> tuple[float, str]:
    ...
    for keyword in SUSPICIOUS_MERCHANT_KEYWORDS:
...
def rule_card_testing(row, baseline: dict, *, velocity_count: int = 0) -> tuple[float, str]:
    """Rapid small charges on a low-spend card — common card-testing fraud."""
    if velocity_count < 6:
        return 0.0, ""
...
def rule_foreign_high_amount(row, baseline: dict) -> tuple[float, str]:
    ...
    if ratio >= 8 and amount >= 300:
        return (
            0.75,
            f"Foreign charge ${amount:,.2f} ({ratio:.0f}× median) — card from {home}, merchant in {merchant_country}",
        )
