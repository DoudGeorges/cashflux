...
from fraud_detector.baseline import build_baselines, build_cross_card_maps
from fraud_detector.rules import (
    rule_amount_anomaly,
...
def combine_rule_scores(hits: dict[str, tuple[float, str]]) -> float:
    ...
    band (roughly 50–92%) so reviewers see differentiated risk, not piles of 100%.
    """
...
def score_transactions(df: pd.DataFrame, threshold: float = DEFAULT_THRESHOLD) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        hits = {
            ...
            "suspicious_merchant": rule_suspicious_merchant(row, baselines, burst_cards=burst_cards),
            "card_testing": rule_card_testing(row, baselines, velocity_count=velocity_count),
            ...
            "new_device": (
                (0.25, sanitize_display_text(f"New device {row['device_id']} first use on this card"))
