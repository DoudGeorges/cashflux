...
FAKE_EMPLOYEES = CANONICAL_EMPLOYEES
...
def value_allowed(actual_value, allowed_values):
    """
    If allowed_values is empty, that field has no restriction.
    If allowed_values contains ANY, it passes.
    Otherwise actual_value must be in the allowed list.
    """
    actual = normalize_location(actual_value)
...
STREET_NAMES = [
    "Main St", "King St", "Queen St", "Market St", "Broadway", "Park Ave",
    "Elm St", "Maple Ave", "Oak St", "Cedar Rd", "Lake Shore Blvd",
    "University Ave", "Industrial Pkwy", "Commerce Dr", "Airport Rd",
    "Victoria St", "Saint Laurent Blvd", "Sherbrooke St", "Yonge St",
    "Front St", "Bloor St", "Rue Sainte-Catherine", "René-Lévesque Blvd"
]
...
def add_specific_addresses_if_needed(df):
    ...
    for i, row in df.iterrows():
        ...
        if (
            normalize_location(approved_city) in ["ANY", "ALL", "*"]
            ...
            if clean_text(row.get("Approved Street Address", "")) == "":
                ...
                if (
                    ...
                    df.at[i, "Approved Street Address"] = fake_street_address(approved_seed)
...
def assign_canonical_employees(df):
    ...
    for i in range(len(df)):
        ...
        if existing_id in EMPLOYEE_BY_ID:
            ...
            employee_id, employee_name, department = CANONICAL_EMPLOYEES[i % len(CANONICAL_EMPLOYEES)]
...
def _pick_merchant(rng, category, city_key):
    ...
    if city_key == "MONTREAL":
        ...
        pool = [m for m in SECONDARY_CITY_MERCHANTS.get(city_key, []) if m["category"] == category]
        if not pool:
...
def assign_real_merchants(df, montreal_share=MONTREAL_SPEND_SHARE):
    ...
    for i in range(len(df)):
        ...
        if "Transaction Amount" in df.columns and str(df.at[i, "Debit or Credit"]).lower() == "debit":
...
def bias_merchant_locations(df, montreal_share=MONTREAL_SPEND_SHARE):
    """Legacy alias — real merchants include location data."""
    return assign_real_merchants(df, montreal_share=montreal_share)


def add_fake_employees_if_needed(df):
    """Legacy wrapper — always normalize to the canonical roster."""
    return assign_canonical_employees(df)
...
def add_fake_geofence_if_needed(df, force=False):
    ...
    for _, row in df.iterrows():
        ...
        # 7%: intentionally different approved location, so geofencing can catch it.
            ...
            approved_city, approved_state, approved_country = random.choice(FAKE_APPROVED_LOCATIONS)
...
def geofence_risk(row):
    ...
    if not no_location_restriction:
        if merchant_city == "" and merchant_state == "" and merchant_country == "":
            ...
            reasons.append("Merchant location is missing, so geofence cannot be verified.")
    ...
    if not no_date_restriction:
        if pd.isna(transaction_date):
            risk = higher_risk(risk, "Low")
            reasons.append("Transaction date is missing or invalid, so approved travel dates cannot be verified.")
        else:
...
def detect_transaction(row):
    ...
    if keyword:
        ...
        credit_reasons.append(f"Possible personal or irrelevant purchase detected: '{keyword}'.")
...
ORIGINAL_COLUMNS_TO_REMOVE = [
    ...
    "geo_flagged", "geo_reason", "approved_location",
    "merchant_location", "geofence_status",
]
...
def rescore_transactions(df):
    ...
    try:
        from policy_engine import check_transaction_policy, detect_split_purchases, load_policy_rules
        ...
        for idx in split_indices:
            if idx in df.index and idx not in flagged_df.index:
                row = df.loc[idx].copy()
                matching = next(
                    (s for s in split_flags if str(idx) in (s.get("transaction_ids") or [])),
                    None,
                )
        ...
        if policy_rows:
            ...
            flagged_df = flagged_df.drop_duplicates(
                subset=["Employee ID", "Transaction Date", "Merchant Info DBA Name", "Amount Clean"],
                keep="first",
            )
    except ImportError:
        pass
...
def main(company_slug=None):
    ...
    save_transaction_outputs(df, scores_df, flagged_df, department_df, paths=company_paths)
