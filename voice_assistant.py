"""Voice assistant — spoken Q&A with full site awareness and navigation actions."""
...
VOICE_VERBOSITY = (
    ...
    "Reply in plain conversational English — 1 to 3 short sentences. "
...
VIEW_PATTERNS: list[tuple[str, tuple[str, ...]]] = [
    ...
    ("receipts", (r"\breceipts?\b", r"\breceipt scan\b", r"\bscan receipt\b", r"\bupload receipt\b")),
    ("proposals", (r"\bmy projects\b", r"\bproposals?\b", r"\bproject proposals?\b", r"\bbudget request\b")),
    ("activity", (r"\bpurchases?\b", r"\btransactions?\b", r"\ball purchases\b", r"\bactivity\b", r"\bmy purchases\b")),
    ("budget", (r"\bbudgets?\b", r"\bburn rate\b", r"\bforecast\b", r"\bdepartment cap\b")),
    ("map", (r"\bmap\b", r"\blocations?\b", r"\bwhere purchases\b")),
    ("alerts", (r"\bproblems?\b", r"\bflags?\b", r"\bflagged\b", r"\bviolations?\b", r"\boffenders?\b")),
    ("approvals", (r"\breview\b", r"\bapprove\b", r"\bapprovals?\b", r"\bpending requests?\b", r"\bwaiting for\b", r"\bfraud\b", r"\bfraud detection\b", r"\bsuspicious purchases?\b", r"\bscam\b", r"\bfraud hunter\b")),
    ("reports", (r"\btrip reports?\b", r"\breports page\b", r"\bcfo reports?\b", r"\bexpense reports?\b")),
    ("chat", (r"\bchat\b", r"\bask anything\b", r"\btext chat\b", r"\bfriday\b")),
    ("settings", (r"\bsettings\b", r"\bspending rules\b", r"\bconfigure\b", r"\bsetup\b")),
]
...
def _normalize_nav_text(text: str) -> str:
    cleaned = text.lower()
    cleaned = re.sub(r"\b(the|a|an|section|page|tab|area|screen|panel|part)\b", " ", cleaned)
...
def _wants_navigation(text: str) -> bool:
    ...
    if re.search(r"\b(go|open|show|view|see)\s+(me\s+)?(the\s+)?", low):
        return True
    if re.search(r"\b(what needs|what's pending|pending approval|waiting for me)\b", low):
...
def _employee_spend_extremes(employee_name: str | None = None) -> tuple[dict | None, dict | None]:
    from expense_data import _sum, format_money, scoped_expenses

    rows = scoped_expenses(employee_name)
    totals = _sum(rows, "employee")
    if not totals:
        return None, None
    ...
    return (
        {"name": top_name, "spend_fmt": format_money(top_amt)},
        {"name": low_name, "spend_fmt": format_money(low_amt)},
    )


def _personal_transaction_extreme(employee_name: str, highest: bool = True) -> dict | None:
    from expense_data import format_money, scoped_expenses

    rows = scoped_expenses(employee_name)
    if not rows:
        return None
    row = max(rows, key=lambda r: r["amount"]) if highest else min(rows, key=lambda r: r["amount"])
    return {
        "amount_fmt": format_money(row["amount"]),
        "vendor": row.get("vendor") or "unknown vendor",
        "date": row["date"].date().isoformat(),
    }


def _personal_total(employee_name: str) -> str | None:
    from expense_data import format_money, scoped_expenses

    rows = scoped_expenses(employee_name)
    if not rows:
        return None
    return format_money(sum(r["amount"] for r in rows))
...
def try_instant_response(
    ...
    if re.search(r"\b(how many|count|number of).*\b(approval|pending)", low):
        ...
        extra = [{"type": "navigate", "view": "approvals"}] if _wants_navigation(low) else []
    ...
    if re.search(r"\b(how many|count|number of).*\b(flag|problem|violation)", low):
        if personal:
            ...
            n = sum(1 for r in scoped_expenses(employee_name) if r.get("flagged") == "yes")
            return _instant_result(f"You have {n} flagged purchases.")
        n = counts["flags"]
        extra = [{"type": "navigate", "view": "alerts"}] if _wants_navigation(low) else []
    ...
    if re.search(r"\b(total spend|overall spend|how much (have we )?spent|company spend)\b", low):
        if personal:
            total = _personal_total(employee_name)
            if total:
                return _instant_result(f"Your total spend is {total}.")
    ...
    if re.search(r"\b(what needs my attention|what's pending|what should i (do|review|look at))\b", low):
        if personal:
            return None
        return _instant_result(
    ...
    if re.search(r"\b(top department|most spend|who spent the most|biggest spender|highest spender)\b", low):
        if personal:
            txn = _personal_transaction_extreme(employee_name, highest=True)
        ...
        if snap["top_departments"]:
            ...
            return _instant_result(f"{top['name']} leads with {top['spend_fmt']} in total spend.")
